...
def pypher(psf_source, psf_target, output, pix_scale):

    angle_source = 0
    angle_target = 0

    kernel_basename, _ = os.path.splitext(output)
    kernel_fits = str(cd)+'/Kernel/'+kernel_basename + '.fits'

    # Load images (NaNs are set to 0)
    psf_source = fits.getdata(psf_source)
    psf_target = fits.getdata(psf_target)
    ...
    pixscale_source = pixscale_target = pix_scale

    # Rotate images (if necessary)
    if angle_source != 0.0:
        psf_source = imrotate(psf_source, angle_source)
    if angle_target != 0.0:
        psf_target = imrotate(psf_target, angle_target)
    ...
    reg_fact = 1E-4
    kernel, _ = homogenization_kernel(psf_target, psf_source, reg_fact)

    # Write kernel to FITS file
    fits.writeto(kernel_fits, data=kernel, clobber=True)
    return kernel
...
def imrotate(image, angle, interp_order=1):
    ...
    """
    return rotate(image, -1.0 * angle, order=interp_order, reshape=False, prefilter=False)


def imresample(image, source_pscale, target_pscale, interp_order=3):
    """
